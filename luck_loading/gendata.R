...
if(EFFECTSIZE_Gendata > 0){
  alpha <- (EFFECTSIZE_Gendata + 0.6)
  betaa <- (1 - alpha*alpha)
  cur1 <- unlist(strsplit(EFFECTSIZE_lst, split=" "))
  strt3 <- "MODEL POPULATION-g2:\n"
  tt3b <- ""
  if((Factornum==1)||(length(cur1)==1)){
    if(length(cur1)==1){
      tt3b <- paste0("f1 by V",cur1[1],"*",alpha,";\n") 
    }else{
      tt3b <- paste0("f1 by V",cur1[1])
      for(i in 2:length(cur1)){
        tt3b <- paste0(tt3b,"-V",cur1[i])
      }    
      tt3b <- paste0(tt3b,"*",alpha,";\n") 
    }
  } else {
    if(Factornum == length(cur1)){
      for(i in 1:length(cur1)){
        tt3b <- paste0(tt3b,"f",i," by V",cur1[i],"*",alpha,";\n")
      }
    }else{
      for(i in 1:Factornum){
        tt3b <- paste0(tt3b,"f",i," by V",cur1[2*i-1],"-V",cur1[2*i],"*",alpha,";\n")
      }
    }
  }
  strt3 <- paste0(strt3,tt3b)
  templatet3 <- "V%s*%f;\n"
  for(i in 1:length(cur1)){
    tt3 <- sprintf(templatet3, cur1[i], betaa)
