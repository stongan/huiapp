...
########################
getModelHeader_merge<-function(cQuesNum, cFactorNum){
  len1 <- as.integer(cQuesNum/cFactorNum)
  ret <- ""
  ret2 <- ""
  template <- "f%d by V%d-V%d;\n"
  ...
  ret3 <- ""
  if(cFactorNum > 1){
    ret3 <- paste0("-f",cFactorNum)
  }
  ret4 <- sprintf(template3, ret3, cQuesNum)

  return (paste0(ret,ret2,ret4))
}

selectv2_merge<-function(cQuesNum, cFactorNum){
  retmodel <- ""
  template1 <- "
  NAMES = V1-V%d;
  USEVARIABLES = V1-V%d;
  "
  model_VARIABLE <- sprintf(template1, (cQuesNum+1), cQuesNum, (cQuesNum+1))
  model_MODEL <- getModelHeader_merge(cQuesNum, cFactorNum)
