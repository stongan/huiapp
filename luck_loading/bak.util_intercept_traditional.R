...
selectreferent_perfactor<-function(cQuesNum, cFactorNum, factoridx, interceptidx){
  ...
  model_MODEL <- sprintf(template2, 
                         ...
                         factoridx,
                         factoridx,
                         interceptidx+usevarbegin-1,interceptidx+usevarbegin-1,
                         freeintercept,
...
selectreferent_perfactor_multi_helper <- function(cQuesNum, cFactorNum, interceptidxlst){
  ...
  for(i in 1:cFactorNum){
    ...
    begin1 <- ((i-1)*len1+1)
    end1 <- len1*i
    c1 <- sprintf(template1, delidx+begin1-1, delidx+begin1-1)
...
selectreferent_perfactor_merge_0<-function(cQuesNum, cFactorNum, interceptidxlst){
  ...
  template3_3 <- "[V1-V%d];"
  str3_3 <- sprintf(template3_3, cQuesNum)
  template4_4 <- "
  %s
  %s
  %s
...
scalar_scale_perfactor<-function(cQuesNum, cFactorNum){
  ...
  model_MODEL <- sprintf(template2, 
                         cQuesNum, #model
                     cQuesNum, cQuesNum, cQuesNum, cQuesNum, cQuesNum, #G1
                     cQuesNum, cQuesNum, cQuesNum, cQuesNum, cQuesNum) #G2
                         
  return (list(model_VARIABLE, model_MODEL))
}

scalar_scale_multifactor<-function(cQuesNum, cFactorNum){
  retmodel <- ""
  template1 <- "
  NAMES = V1-V%d;
  USEVARIABLES = V1-V%d;
...
scalar_item_helper_g1 <- function(begin1, end1, fixinterceptidx){
  ret <- ""
  template1 <- "[V%d-V%d](I%d-I%d);"
  template2 <- "[V%d](I%d);[V%d-V%d](I%d-I%d);"
  template3 <- "[V%d-V%d](I%d-I%d);[V%d](I%d);"
  template4 <- "[V%d-V%d](I%d-I%d);[V%d-V%d](I%d-I%d);"
  if (begin1 == fixinterceptidx) {
    ret <- sprintf(template1, begin1+1, end1, begin1+1, end1)
  } else if(end1 == fixinterceptidx) {
    ret <- sprintf(template1, begin1, end1-1, begin1, end1-1)
  } else if(begin1 == fixinterceptidx-1){
    ret <- sprintf(template2, begin1, begin1, fixinterceptidx+1, end1, fixinterceptidx+1, end1)
  } else if(end1 == fixinterceptidx+1){
    ret <- sprintf(template3, begin1, fixinterceptidx-1, begin1, fixinterceptidx-1, end1, end1)
  } else {
    ret <- sprintf(template4, begin1, fixinterceptidx-1, begin1, fixinterceptidx-1, fixinterceptidx+1, end1, fixinterceptidx+1, end1)
  }
  return (ret)
}

scalar_item_helper_g2 <- function(begin1, end1, fixinterceptidx){
  ret <- ""
  template1 <- "[V%d-V%d](H%d-H%d);"
  template2 <- "[V%d](H%d);[V%d-V%d](H%d-H%d);"
  template3 <- "[V%d-V%d](H%d-H%d);[V%d](H%d);"
  template4 <- "[V%d-V%d](H%d-H%d);[V%d-V%d](H%d-H%d);"
  if (begin1 == fixinterceptidx) {
    ret <- sprintf(template1, begin1+1, end1, begin1+1, end1)
  } else if(end1 == fixinterceptidx) {
    ret <- sprintf(template1, begin1, end1-1, begin1, end1-1)
...
scalar_item_perfactor<-function(cQuesNum, cFactorNum, fixques, testitem){
  ...
  "
  model_VARIABLE <- sprintf(template1, (cQuesNum+1), cQuesNum, (cQuesNum+1))
  g1str <- scalar_item_helper_g1(1, cQuesNum, fixques)
  g2str <- scalar_item_helper_g2(1, cQuesNum, fixques)
  ...
  model_MODEL <- sprintf(template2, 
                         cQuesNum, #model
                     cQuesNum, cQuesNum, fixques, fixques, g1str, cQuesNum, #G1
                     cQuesNum, cQuesNum, fixques, fixques, g2str, cQuesNum) #G2

  templates <- "0=I%d-H%d;\n"
  model_constraint <- sprintf(templates, testitem, testitem)
                         
  return (list(model_VARIABLE, model_MODEL, model_constraint))
}

scalar_item_multifactor_helper <- function(cQuesNum, cFactorNum, fixques){
  template1 <- "[V%d](I%d);\n"
  retg1 <- ""
  retg2 <- ""
  len1 <- as.integer(cQuesNum/cFactorNum)
  for(i in 1:cFactorNum){
    ...
    end1 <- len1*i
    c1 <- sprintf(template1, delidx+begin1-1, delidx+begin1-1)
    str1 <- scalar_item_helper_g1(begin1, end1, delidx+begin1-1)
    str2 <- scalar_item_helper_g2(begin1, end1, delidx+begin1-1)
    retg1 <- paste0(retg1,c1)
    retg1 <- paste0(retg1,str1)
    retg2 <- paste0(retg2,c1)
    retg2 <- paste0(retg2,str2)
  }
  return (list(retg1,retg2))
}

scalar_item_multifactor<-function(cQuesNum, cFactorNum, fixques, testitem){
  retmodel <- ""
  template1 <- "
  NAMES = V1-V%d;
  USEVARIABLES = V1-V%d;
  ...
  g1g2str <- scalar_item_multifactor_helper(cQuesNum, cFactorNum, fixques)
  g1strlong <- unlist(g1g2str[1])
  g2strlong <- unlist(g1g2str[2])
...
##这是一个BIG Project，函数命名尽可能保证唯一性，Thx
abcsepAndMergeEst1st <- function(lst1, fname, fixidxlst, cfactornum, resfile, model_var, model_model, model_constra, rstep) {
  library(MplusAutomation)
  ##设置输出模型到指定的文件夹
  setwd(resfile)
  ##拼接输出文件的名字及中间结果参数
  ...
  retfile1 <- paste0(fname,"_",rstep,".inp")
  #if(rstep == 2){
  #  retfile1 <- paste0(fname,".inp")
  #}
