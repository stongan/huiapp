...
estimateCore <- function(lst1, fname, idx, f1, resfile,tmodelstr) {
  library(MplusAutomation)
  setwd(resfile)
...
callEstimate <- function(estlst, f12, randnum, resfile, idx, minidx) {
  ##设置模型生成的路径分G1和G2跑 
  ...
  tmodelstr <- getCurEqual2nd(minidx)
  ##遍历每一个文件
  for(r in 1:nrow(tmatrix)){
    ...
    #fileone no using
    #参数：数据、nouse、表情、文件名、输出文件路径
    estimateCore(cur2man,fileone,1,f2,t_outG1,tmodelstr)
    estimateCore(cur2fem,fileone,1,f2,t_outG2,tmodelstr)
  }
}
...
##注意修改重复次数
callMplus <- function(estlst, f1, randnum, resfile, idx, minidx) {
  library(MplusAutomation)
  ##新建模拟数据的保存路径并设置工作路径
  ...
  for(i in 1:length(estlst)){
    yvalue <- i
    if(i == minidx){
      t_popu <- paste0(t_popu,"y",yvalue,"@1\n") 
    } else if(i < 11){
  ...
  t_popu <- paste0(t_popu, "; f1*",estlst[11],"; [f1@0];")
  ##执行模拟并生产数据
  pathmodel <- mplusObject(
...
mainfunc2rd <- function(curpath, idx) {
  ...
  randomlst <- c(sample(1000:100000, length(names(paraG12)), replace = F))
  ##并发操作需要的函数,需要显示指定
  funclst <- c("callMplus","callEstimate","estimateCore","G1diffG2","G1dG2Core","NREPS","Questionnum","getCurEqual2nd")
  ##遍历路径下的每一个.out文件
  ##for(i in 1:length(names(paraG12))){
  foreach(i=1:length(names(paraG12)), .verbose=T, .export=funclst, .combine="c") %dopar% {
    ...
    ##切割文件名字，只要后缀名之前的一段
    cur1 <- unlist(strsplit(curname, split="\\."))
    cur2 <- cur1[length(cur1)-1]
    cur3 <- unlist(strsplit(cur2, split="__"))
    cname[i] <- cur3[length(cur3)-1]
    minidx <- as.numeric(cur3[length(cur3)])
    ##抽每个out文件的参数
    estlst <- paraG12[[curname]]$parameters$unstandardized$est
    ##按文件名保存到矩阵
    allret[cname[i]] <- as.vector(estlst)
    ##1-1 调用mplus生成模拟数据
    callMplus(estlst, cname[i], randomlst[i], curpath, idx, minidx)
    ##1-2 根据上面的模拟数据再跑一次模型
    callEstimate(estlst, cname[i], randomlst[i], curpath, idx, minidx)
    ##1-3 计算每一个G1diffG2
