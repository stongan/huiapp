...
dodiff2 <- function() {
  ...
  write.csv(metric0,file=paste0(outMCFA,"/metric0_origin.csv"))
  write.csv(scalar1,file=paste0(outMCFA,"/scalar1_origin.csv"))
  
  ##进一步抽参数
  metric0 <- data.matrix(metric0[c("ChiSqM_Value","ChiSqM_DF","CFI","TLI","AIC","BIC","RMSEA_Estimate","SRMR"),])
  scalar1 <- data.matrix(scalar1[c("ChiSqM_Value","ChiSqM_DF","CFI","TLI","AIC","BIC","RMSEA_Estimate","SRMR"),])
  ##做相减
...
dodiff3 <- function() {
  ...
  col_set <- c("NGroups", "NDependentVars", "NIndependentVars", "NContinuousLatentVars", "Parameters", "ChiSqM_Value", "ChiSqM_DF", "ChiSqM_PValue", "ChiSqBaseline_Value", "ChiSqBaseline_DF", "ChiSqBaseline_PValue", "LL", "UnrestrictedLL", "CFI", "TLI", "AIC", "BIC", "aBIC", "RMSEA_Estimate", "RMSEA_90CI_LB", "RMSEA_90CI_UB", "RMSEA_pLT05", "SRMR")
  len_col_set <- length(col_set)
  ret1 <- data.frame(matrix(NA,len_col_set,0))
  ...
  for(i in 1:length(names(item2))){
    ...
    base1 <- data.matrix(base1[col_set,])
    exp1 <- data.matrix(exp1[col_set,])
    diff1 <- exp1 - base1
    ret1[curname] <- diff1 
  }
