...
estimateCore <- function(lst1, f1, resfile) {
  library(MplusAutomation)
  ##设置输出模型到指定的文件夹
  setwd(resfile)
  ##拼接输出文件的名字及中间结果参数
...
callEstimate <- function(f12, resfile) {
  ...
  f2 <- filetmpgroup[1]
  firstfit <- estimateCore(cur2,f2,t_outG1)
  ...
  for(r in 1:nrow(tmatrix)){
    ...
    #参数：数据、文件名、输出文件路径
    curfit <- estimateCore(cur2,f2,t_outG1)
    if(r == 1){
      curfit <- firstfit
    }
    curallret <- getdiff2nd(curfit)
    curdiff <- curallret[1]
...
callMplus <- function(estlst, f1, randnum, resfile, idx) {
  ...
  ##循环凭借参数
  t_popu <- "f1 by "
  for(i in 1:length(loadlst)){
    t_popu <- paste0(t_popu,"y",i,"*",loadlst[i],"\n")
  }
  ...
  if(Factornum>1){
    for (i in 2:Factornum) {
      for (j in 1:(i-1)) {
        t_popu<-paste0(t_popu,"f",i," with ","f",j,";","\n")
      }
    }
  }
  print (t_popu)
...
mainfunc2rd <- function(curpath, idx) {
  ...
  randomlst <- c(sample(1000:100000, length(names(paraG12)), replace = F))
  ##并发操作需要的函数,需要显示指定
  funclst <- c("callMplus","callEstimate","estimateCore","G1diffG2","G1dG2Core","NREPS","loadingnum","factormeannum","interactionnum","interceptnum","varinum","residualnum","Questionnum","Factornum","HasInterception","HasFactormean","NGroup1st","Group1_samplesize","Group2_samplesize","getFactorCorr","getdiff2nd")
  ##遍历路径下的每一个.out文件
  ##for(i in 1:length(names(paraG12))){
