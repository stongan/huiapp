...
callEstimate <- function(f12, resfile) {
  ...
  print (f12)
  print ("f12")
  minlstgroup <- unlist(strsplit(f12, split="_"))
  minlstlen <- as.integer(minlstgroup[2])
  minlst <- rep(1,minlstlen)
  for(k in 3:length(minlstgroup)){
    minlst[k-2] <- as.integer(minlstgroup[k])
  }
  print (minlst)
  print (length(minlst))
  ...
  for(r in 1:nrow(tmatrix)){
    ...
    if(Factornum==1){
      curselectret_sep <- selectreferent_perfactor(Questionnum, 1, 1, minlst[1])
    } else {
      curselectret_sep <- selectreferent_perfactor_multifactor(Questionnum, Factornum, minlst)
    }
    model_var_sep <- unlist(curselectret_sep[1])
    model_model_sep <- unlist(curselectret_sep[2])
    curfit_sep <- sepAndMergeEst1st(cur2, f2, minlst, Factornum, t_outG1, model_var_sep, model_model_sep, 2)
...
mainfunc2rd <- function(curpath, idx) {
  ...
  randomlst <- c(sample(1000:100000, length(names(paraG12)), replace = F))
  ##并发操作需要的函数,需要显示指定
  funclst <- c("callMplus","callEstimate","estimateCore","G1diffG2",
