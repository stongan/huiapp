...
for(r in 1:nrow(tmatrix)){
  ...
  for(dfidx in 1:Questionnum) {
    ...
    ##curdeta是Questionnum维向量
    tmpmin <- 10000
    tmpminidx <- 10000
    for(tmpidx in 1:Questionnum) {
      if(dfidx != tmpidx){
        if(tmpmin > curdeta[tmpidx]){
          tmpmin = curdeta[tmpidx]
          tmpminidx = tmpidx
        }        
      }
    }
    sumdeta[tmpminidx] <- sumdeta[tmpminidx]+1
  }
  minIdx = 1
  maxcnt_t = 0
  for(k in 1:Questionnum){
    if(sumdeta[k] > maxcnt_t){
      maxcnt_t = sumdeta[k]
      minIdx = k
    }
  }
