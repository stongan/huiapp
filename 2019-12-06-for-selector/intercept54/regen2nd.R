...
mainfunc2rd <- function(curpath, idx) {
  ...
  system.time(
    res2.p <- foreach(i=1:cores, .combine='rbind', .export=funclst) %dopar%
    {
    ##遍历路径下的每一个.out文件
      for(x in ((i-1)*chunk.size+1):(i*chunk.size)) {
        ...
        setwd(scriptpath)
        ##取当前文件名字
        curname <- names(paraG12)[x]
        ##切割文件名字，只要后缀名之前的一段
        cur1 <- unlist(strsplit(curname, split="\\."))
        cname[x] <- cur1[length(cur1)-1]
        ##抽每个out文件的参数
        estlst <- paraG12[[curname]]$parameters$unstandardized$est
        ##按文件名保存到矩阵
        allret[cname[x]] <- as.vector(estlst)
        ##1-1 调用mplus生成模拟数据
        callMplus(estlst, cname[x], randomlst[x], curpath, idx)
        ##1-2 根据上面的模拟数据再跑一次模型
        callEstimate(estlst, cname[x], randomlst[x], curpath, idx)
        ##1-3 计算每一个G1diffG2
        G1diffG2(estlst, cname[x], randomlst[x], curpath, idx)
      }
    }
  ) 
  ##保存中间数据-抽取出来估计参数矩阵 
