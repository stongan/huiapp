...
##for(r in 1:nrow(tmatrix)){
  ...
  for(factoridx in 1:Factornum){
    sumdeta <- rep(0.0,as.integer(Questionnum/Factornum))
    for(interceptidx in 1:as.integer(Questionnum/Factornum)) {
      curselectret <- selectreferent_perfactor(Questionnum, Factornum, factoridx, interceptidx)
      model_var <- unlist(curselectret[1])
      model_model <- unlist(curselectret[2])
      curfit <- estForSelect1st(cur2,f2,interceptidx, factoridx, selerefpath, model_var, model_model)
      talllst <- dodiffbyparam1st(curfit,as.integer(Questionnum/Factornum),0,1,as.integer(Questionnum/Factornum),1,as.integer(Questionnum/Factornum))
                                  #fit1, tloadingnum, tinteractionnum, tfactormeannum, tinterceptnum, tvarinum, tresidualnum
      
      g1interceplst <- unlist(talllst[4])
      g2interceplst <- unlist(talllst[10])
      diffinterceptref<- abs(g1interceplst-g2interceplst)
      sumdeta <- sumdeta + diffinterceptref
    }
    sumdeta <- sumdeta / (as.integer(Questionnum/Factornum)-1)
    minIdx = 1
    print (sumdeta)
    print (class(sumdeta))
    for(k in 1:as.integer(Questionnum/Factornum)){
      tofind <- as.character(min(sumdeta))
      if(as.character(sumdeta[k]) == tofind){
        minIdx = k
      }
    }
    factor_minlst[factoridx] <- minIdx
  }
